...
{
    [BroadcastReceiver]
    public class RepeatingAlarmTenSec : BroadcastReceiver
    {
        public override void OnReceive(Context context, Intent intent)
        {
            //Every time the `RepeatingAlarm` is fired, set the next alarm
            var intentForRepeat = new Intent(context, typeof(RepeatingAlarmTenSec));
            ...
            am.SetExactAndAllowWhileIdle(AlarmType.ElapsedRealtimeWakeup, SystemClock.ElapsedRealtime() + 10 * 1000, source);

            Toast.MakeText(context, "пора пить таблетки 10sec", ToastLength.Long).Show();
        }
    }
}